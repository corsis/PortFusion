...
| |
| <p>Connections to `home:3389` will now be tunnelled and reach `work:3389` circumventing corporate firewalls.</p> <p>All you need to do is make `home:2000` accessible from `work`.</p> | <p>`local` HTTP clients that connects to the tunneled proxy `local:3128` are served via gateway `remote` by your friend's http proxy `server:3128`.</p> <p>Your friend only needs to configure the firewall on gateway `remote` for port `2000`.</p>
| |
